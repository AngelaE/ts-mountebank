...
describe("The flexi predicate", () => {
    ...
    const tests = [
        {
            operator: Operator.startsWith, predicatePath: '/test',
            matches: ['/testpath'], nonMatching: ['/te']
        },
        {
            operator: Operator.endsWith, predicatePath: 'ong',
            matches: ['/testpath-long'], nonMatching: ['/testing']
        },
    ]
        ...
        describe(`${test.operator} Operator with path '${test.predicatePath}'`, () => {
            before(async () => {
                let imposter = new Imposter().withPort(port).withStub(
                    new Stub().withPredicate(new FlexiPredicate()
