...
describe("Proxy", () => {
    ...
    describe("Proxies work in Mountebank", async () => {
        ...
        const proxyTestCases = [
            {mode: ProxyMode.ProxyAlways, expectedSecondResponseStatus: secondImposterResponseStatus},
            {mode: ProxyMode.ProxyOnce, expectedSecondResponseStatus: firstImposterResponseStatus}
        ]
        ...
        it(`can create a debug proxy for mode ${test.mode}`, async () => {
            ...
            const body = 'yippie';
            let responseImposter = new Imposter().withPort(port).withStub(new DefaultStub(testPath, HttpMethod.GET, body, firstImposterResponseStatus));
            let newImposter = new Imposter().withPort(port).withStub(new DefaultStub(testPath, HttpMethod.GET, body, secondImposterResponseStatus));

            const proxyPort = port + 1;

            // act
            let proxyImposter = new Imposter().withPort(proxyPort)
                .withStub(new Stub().withProxy(new DebugProxy(`http://localhost:${port}`).withMode(test.mode)));
            ...
            // assert
            let response = await request.get(`http://localhost:${proxyPort}${testPath}`);
            ...
            response = await request.get(`http://localhost:${proxyPort}${testPath}`);
